def is_valid (str)
    ...
    # if user has input a char whose occurance in string is just once and the occurance of other char in input is same
    if grp_char_occurance.length == 1 || (grp_char_occurance.length <=2 and grp_char_occurance.any?(1))
        return "YES"
    else
        return "NO"
    end
